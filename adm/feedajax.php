...
if (isset($_POST['photo_id'])) {
    ...
    $dir = (int)$_POST['id'];
    unlink(UPLOADS_PATH . '/feed/' . $dir . '/' . $photo);
    unlink(UPLOADS_PATH . '/feed/' . $dir . '/mini/' . $photo);
    $db->simple_query("UPDATE `".DB_PREFIX."_feed` SET `count`=`count`-1 WHERE `id`=".$dir);
}
...
if ($_FILES) {
    ...
    if (count($_FILES['filesToUpload'])) {
        ...
        foreach ($x4 as $key => $value) {
            ...
            $db->simple_query("UPDATE `".DB_PREFIX."_feed` SET `count`=`count`+1 WHERE `id`=".$gallery_id);
        }
    }
