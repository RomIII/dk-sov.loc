...
$action = 'banner.php'; // название этого файла
$table_name = DB_PREFIX . '_banner'; // название таблицы с которой работаем
$folder = 'banner';
$content='';
...
$formItems = array(
    array('title' => 'название', 'type' => 'text', 'name' => 'name', 'value' => '', 'save' => 'text'),
    array('title' => 'ссылка', 'type' => 'text', 'name' => 'url', 'value' => '', 'save' => 'text'),
    array('title' => 'подпись', 'type' => 'text', 'name' => 'alt', 'value' => '', 'save' => 'text'),
);
...
if ($a == '') {
    ...
    if ($result = $db->mysqli->query('SELECT * FROM `' . $table_name . '` ORDER BY `nom`')) {
        while ($row = $result->fetch_assoc()) {
            ...
            $row['class'] = $class;
            $content .= Banner::admItem($row, $action,$folder);
        }
    }
    $content .= '</ul>';
}
...
$templater = array(
    '%_TITLE_%' => 'Администрирование : баннеры',
